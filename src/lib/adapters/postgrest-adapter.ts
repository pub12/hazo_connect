 ...
 */
export class PostgrestAdapter extends BaseAdapter implements HazoConnectAdapter {
  private postgrestConfig: PostgrestConfig
  ...
  constructor(config: any, logger?: Logger) {
    ...
    if (!this.postgrestConfig.base_url || !this.postgrestConfig.api_key) {
      this.throwError(
        EC.CONFIG_ERROR,
        'PostgREST base_url and api_key are required in configuration'
      )
    }
   ...
   */
  private buildPostgrestUrl(builder: QueryBuilder): string {
    const table = builder.getTable()
    if (!table) {
      this.throwError(EC.VALIDATION_ERROR, 'Table name is required')
    }
  ...
  async rawQuery(endpoint: string, options: RequestInit = {}): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const friendlyMessage = this.buildFriendlyErrorMessage(
          response.status,
          response.statusText,
          endpoint,
          parsedDetails,
      ...
      this.throwError(
        EC.QUERY_ERROR,
        `PostgREST request error: ${error.message || String(error)}`,
        undefined,
        error
      )
    }
   ...
   */
  async getConfig(): Promise<PostgrestConfig> {
    return Promise.resolve(this.postgrestConfig)
  }
  private parseErrorPayload(raw: string) {
    if (!raw) {
      return undefined
    }
  ...
  private buildFriendlyErrorMessage(
    ...
    const detailMessage =
      parsedDetails && typeof parsedDetails === 'object' && parsedDetails.message
