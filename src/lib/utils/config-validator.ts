 ...
 * ```
 */
export function validateNextJsConfig(): ValidationResult {
  const errors: string[] = []
  ...
  if (dbType === 'sqlite') {
    ...
    if (!sqlitePath) {
      warnings.push(
        'HAZO_CONNECT_SQLITE_PATH not set. ' +
  ...
  if (dbType === 'postgrest') {
    if (!process.env.POSTGREST_URL && !process.env.postgrest_url) {
      errors.push(
        'POSTGREST_URL environment variable is required for PostgREST adapter. ' +
    ...
    if (!process.env.POSTGREST_API_KEY && !process.env.postgrest_api_key) {
      errors.push(
        'POSTGREST_API_KEY environment variable is required for PostgREST adapter. ' +
 ...
 */
export function getConfigurationTips(): string[] {
  const tips: string[] = []

  // Check if sql.js might be bundled
  if (typeof window !== 'undefined') {
    tips.push(
      '⚠️  You are in a browser environment. ' +
  ...
  tips.push(
    '💡 Tip: Add sql.js to serverComponentsExternalPackages in next.config.mjs',
    '💡 Tip: Add sql.js to webpack.externals for server-side code',
    '💡 Tip: Use API routes instead of calling hazo_connect directly in Server Components',
    '💡 Tip: Use singleton pattern for multiple API routes (getHazoConnectSingleton)'
  )
