 ...
 */
function extractPostgrestApiKey(): string {
 ...
 */
function extractPostgrestConfig(configProvider?: ConfigProvider): any {
 ...
 */
function extractConfigFromProvider(configProvider: ConfigProvider, type: string): any {
  if (type === 'postgrest') {
 ...
 */
export function createHazoConnect(config: HazoConnectConfig): HazoConnectAdapter {
    ...
    if (type === 'postgrest') {
      ...
      if (providerConfig.sqlite?.database_path) {
        ...
        if (!path.isAbsolute(dbPath) && !dbPath.startsWith(':memory:')) {
          providerConfig.sqlite.database_path = path.resolve(process.cwd(), dbPath)
        }
        ...
        if (path.isAbsolute(envPath)) {
          ...
          providerConfig.sqlite.database_path = path.resolve(process.cwd(), envPath)
        }
      }
    }
  }
  ...
  switch (type) {
    case 'postgrest':
      adapter = new PostgrestAdapter(providerConfig, logger)
      break

    case 'supabase':
      adapter = new SupabaseAdapter(providerConfig, logger)
      break

    case 'sqlite':
      adapter = new SqliteAdapter(providerConfig, logger)
      // If admin UI is enabled, register this adapter so admin service uses the same instance
      if (config.enable_admin_ui === true && adapter instanceof SqliteAdapter) {
        registerSqliteAdapter(adapter)
      }
      break

    case 'file':
      adapter = new FileAdapter(providerConfig, logger)
      break
